...
success_count = 0
fail_count = 0
skipped_nan_count = 0  # 因 NaN 跳过的测试计数
        ...
        for pass_name, pass_df in [("原始", history.copy()), ("预处理", history.copy())]:
            ...
            if pass_name == "预处理":
                if target_nan_count > 0:
                    ...
                    print(f"     [{pass_name}] 目标列预处理: 填充 {target_nan_count} 个NaN")
    ...
    for scene_prefix in ["S1", "S2", "S3"]:
        ...
        if r_raw and not r_raw["success"]:
            if "API不支持NaN输入" in str(r_raw.get("error", "")):
                print(f"      {r_raw['scene'].replace('[原始]',''):>14s} (原始): API不支持NaN输入")
            else:
                print(f"      {r_raw['scene'].replace('[原始]',''):>14s} (原始): 报错 ({r_raw.get('error', '')[:30]})")
